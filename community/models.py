...
class AlumniStory(models.Model):
    author = models.ForeignKey(User, on_delete=models.CASCADE, limit_choices_to={'role': 'alumni'})
...
class MentorResource(models.Model):
    mentor = models.ForeignKey(User, on_delete=models.CASCADE, limit_choices_to={'role': 'professor'})
...
class Mentorship(models.Model):
    mentor = models.ForeignKey(User, on_delete=models.CASCADE, related_name='mentees', limit_choices_to={'role': 'professor'})
    students = models.ManyToManyField(User, related_name='mentorships', limit_choices_to={'role': 'student'})
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Mentor: {self.mentor.username}"
